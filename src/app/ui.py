...
class NumberSelector(BoxLayout):
    ...
    def _post_init(self, *args):
        ...
        for i in range(1, self.sudoku_size + 1):
            self.add_widget(MDLabel(text=str(i), halign='center'))
...
class SudokuBoard(FloatLayout):
    ...
    def _render_numbers(self, **kwargs):
        ...
        for i in range(self.sudoku_size):
            for j in range(self.sudoku_size):
                ...
                image = MDLabel(
                    text=str(self._sudoku_data[i, j]['value']),
    ...
    def set_cell_value(self, selected_value: int, dt: float):
        Logger.info(f'NumberSelector: Selected value: {selected_value}, {self._highlighted_cell}')
...
class SudokuScreen(Screen):
    def on_enter(self, *args):
        super().on_enter(*args)
        # NOTE(nathan): This is for Testing purposes so that I do not have to select a new level from selection
        # screen every time remove in later versions
        Clock.schedule_once(lambda x: self.ids.get('sudoku_board').load_puzzle(3))
